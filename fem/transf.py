...
class ElementTransformation(object):
    ...
    def eval_transf_quant(self, xe, lfcnsp, zv=None, zf=None, zd=None):
        """
        Evaluate the transformation quantities for the given element.

        Args:
            xe (ndarray): The element nodes from the physical domain.
            lfcnsp (object): The object containing the information about the element's shape functions.
            zv (ndarray, optional): The quadrature nodes inside lfcnsp. Defaults to None.
            zf (ndarray, optional): The quadrature nodes on the element faces. Defaults to None.
            zd (ndarray, optional): The quadrature nodes on the element edges. Defaults to None.
        ...
        # Start volume
        # Generate xq (the quadrature nodes in ref domain mapped to physical domain)
        ...
        # Generate Gi (the inverse of the deformation gradient of the isoparametric mapping, evaluated at each quadrature node)
        self.Gi = np.zeros((self.ndim, self.ndim, self.nqpe))
        for i in range(self.nqpe):
            self.Gi[:, :, i] = np.linalg.inv(np.matmul(self.xe, self.lfcnsp.Qvv[:, 1:1+self.ndim, i]))

        
        self.Ge = np.zeros((self.ndim, self.nqpe))
        for i in range(self.nqpe):
            self.Ge[:, i] = np.matmul(self.xe, self.lfcnsp.Qvv[:, 0, i][:, None]).squeeze()
