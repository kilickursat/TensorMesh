...
from fem.check import is_type
from fem.ndist_mltdim import ndist_mltdim_hcube, ndist_mltdim_simp
from fem.tens_core import linidx_from_mltidx, tensprod_vector_unif,\
							   mltidx_from_linidx
...
class Geometry(object):
	"""
	Represents a geometric object in a multi-dimensional space.

	Attributes:
		ndim (int): The number of dimensions of the geometric object.
		porder (int): The polynomial order used for interpolation.

	Methods:
		get_nodes_on_face(f): Returns the nodes on a specified face of the geometric object.
	"""
	...
	def get_nodes_on_face(self, f):
		"""
		Returns the nodes on a specified face of the geometric object.

		Args:
			f (int): The index of the face.

		Returns:
			numpy.ndarray: An array of node indices on the specified face.
		"""
		return self.zk[:, self.f2n[:, f]]

class Hypercube(Geometry):
	"""
	Represents a hypercube geometry in a multi-dimensional space.

	Parameters:
	- ndim (int): The number of dimensions of the hypercube.
	- porder (int): The polynomial order of the hypercube.
	- ndist0 (str, optional): The type of distribution for the initial nodal positions. Defaults to 'unif'.

	Attributes:
	- zk: The z-coordinates of the nodes.
	- N: The normal vectors of the faces.
	- v2n: The vertex-to-node connectivity.
	- d2n: The edge-to-node connectivity.
	- f2n: The face-to-node connectivity.
	- e2n: The element-to-node connectivity.
	"""
...
class Simplex(Geometry):
	'''
	Represents a simplex geometry in multi-dimensions.
	
	Args:
		ndim (int): The number of dimensions of the simplex.
		porder (int): The polynomial order of the simplex.
		ndist0 (str, optional): The type of distribution for the nodes. Defaults to 'unif', supports 'cheb', 'gl' and 'gll'.
	
	Attributes:
		zk (array-like): The coordinates of the nodes.
		N (int): The normal vectors of the faces.
		v2n (array-like): The vertex-to-node mapping.
		d2n (array-like): The edge-to-node mapping.
		f2n (array-like): The face-to-node mapping.
		e2n (array-like): The element-to-node mapping.
	'''
	def __init__(self, ndim, porder, ndist0='unif'):
