...
if __name__ == '__main__':
    ...
    xk = np.linspace(-1, 1, 4)
    x = np.linspace(-1, 1, 100)
    Q = eval_poly_nodal_onedim_vander(xk, x, 2, 0)
    L = eval_poly_nodal_onedim_lagr(xk, x, 0)
    #print Q.shape
    ...
    for k in range(Q.shape[0]):
        plt.subplot(1, 2, 1)
        plt.plot(x, Q[k, 0, :], lw=1)
        plt.title('Vandermonde polynomials')
    # plot the Lagrange polynomials and vander for comparision
    for k in range(L.shape[0]):
        plt.subplot(1, 2, 2)
        plt.plot(x, L[k, 0, :], lw=1)
