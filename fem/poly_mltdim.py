...
from fem.poly_onedim import eval_poly_nodal_onedim_lagr
from fem.tens_core import tensprod_scalar,      \
...
if __name__ == '__main__':
    ...
    coeffs, expnts = get_monom_coeffs_expnts_mltdim(2, 2, 'P', 0)
    
    coeffs, expnts = get_monom_coeffs_expnts_mltdim(2, 2, 'Q', 0)
    coeffs, expnts = get_monom_coeffs_expnts_mltdim(2, 2, 'PQ', 0)
    ...
    xk0 = np.array([[0, 1, 0], [0, 0, 1]], dtype=float)
    x = np.array([[0, 0.5, 0, 1], [0, 0.5, 1, 0]], dtype=float)
    Q = eval_poly_nodal_mltdim_vander(xk0, x, 'P', 0, 0)
    print(Q.shape)